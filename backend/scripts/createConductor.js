...
async function createConductor() {
  try {
    ...
    const password = 'conductor123'; // Change this to desired password
    ...
    if (existing.length > 0) {
      console.log('\n⚠️ User already exists. Updating password and details...');
      await db.query(
        `UPDATE users 
         SET password_hash = ?, role = ?, full_name = ?, phone = ?
         WHERE username = ?`,
        [hashedPassword, 'conductor', fullName, phone, username]
      );
      console.log('✅ Conductor account updated successfully!');
    } else {
      console.log('\n✅ Creating new conductor account...');
      await db.query(
